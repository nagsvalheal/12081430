...
get hasImage() {
    ...
    try {
        if (this.caseImageURL && this.caseImageURL.data) {
            console.log('caseImageURL data:', this.caseImageURL.data); // Log to check the data
			 if(this.caseImageURL.data.length === 0){

		if (this.noPhoto === false) {
				this.noPhotorecords = false;
			}
			else {
				this.noPhotorecords = true;
			}
            ...
            splitArray.forEach(record => {
                if (record) {
                    console.log('Valid record:', record); // Log valid records
